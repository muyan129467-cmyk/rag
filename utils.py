            ...
            if line_len > max_token_len:
                ...
                for i in range(num_chunks):
                    ...
                    if i > 0 and chunk_text:
                        ...
                        # 按token数取覆盖部分
                        actual_cover = min(cover_content, len(prev_tokens))
                        cover_tokens = prev_tokens[-actual_cover:]
                ...
                if chunk_text:
                    ...
                    cover_tokens = prev_tokens[-actual_cover:]
                    cover_part = enc.decode(cover_tokens)
                    cover_len = actual_cover
                    
                    # 检查添加覆盖内容后是否会超过最大长度
                    if cover_len + 1 + line_len <= max_token_len:
                        curr_chunk = cover_part + '\n' + line
                        curr_len = cover_len + 1 + line_len
                    else:
                        # 如果添加覆盖会超限，尝试减少覆盖长度
                        # 从实际覆盖长度开始递减，直到找到合适的长度
                        for reduced_cover in range(actual_cover, 0, -10):  # 每次减少10个tokens
                            if reduced_cover + 1 + line_len <= max_token_len:
                                cover_tokens = prev_tokens[-reduced_cover:]
                                cover_part = enc.decode(cover_tokens)
                                curr_chunk = cover_part + '\n' + line
                                curr_len = reduced_cover + 1 + line_len
                                break
                        else:
                            # 如果无论如何都会超限，则不加覆盖内容
                            curr_chunk = line
